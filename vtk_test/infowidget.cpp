	...
	: QWidget(parent)
{
	...
	ui.button_saveProbe_record->setEnabled(false);
	...
	connect(ui.button_saveProbe, SIGNAL(clicked()), this, SLOT(slot_saveDataProbe()));
	connect(ui.button_saveProbe_record, SIGNAL(clicked()), this, SLOT(slot_recordDataProbe()));
}
...
void InfoWidget::slot_saveDataProbe() {
	fileNameProbe.clear();
	fileNameProbe = QFileDialog::getSaveFileName();
	
	if (!fileNameProbe.isNull()) 
		if (!fileNameProbe.isEmpty())
			ui.button_saveProbe_record->setEnabled(true);
}

void InfoWidget::slot_recordDataProbe() {
	recordingProbe = !recordingProbe; 
	if (recordingProbe) {
		ui.button_saveProbe_record->setText("Stop Recording");
		ui.button_saveProbe->setEnabled(false);
		//qDebug() << "emit sgn_ProveSaveStop()";
		emit sgn_ProveSaveInit(fileNameProbe);
	}
	else {
		ui.button_saveProbe_record->setText("Start Recording");
		ui.button_saveProbe->setEnabled(true);
		emit sgn_ProveSaveStop();
	}
